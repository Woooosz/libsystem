...
CREATE TRIGGER `tr_delete_reader_returnd` AFTER DELETE ON `reader` FOR EACH ROW delete from borrow where rno = old.rno;
CREATE TRIGGER `tr_reader_delete_returnd` BEFORE DELETE ON `reader` FOR EACH ROW delete from returnd where rno = old.rno;